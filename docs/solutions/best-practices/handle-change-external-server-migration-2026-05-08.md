...
`profilePage`는 최소한 위 shape를 유지하고, 필요하면 `userId`나 `updatedAt`을 추가해도 된다.
...
같은 handle을 다시 보내는 경우는 idempotent no-op으로 처리하는 편이 좋다. 즉, 이미 canonical handle이면 200으로 현재 상태를 돌려주면 된다.
...
- redirect는 server가 아니라 호출 측에서 처리한다.
