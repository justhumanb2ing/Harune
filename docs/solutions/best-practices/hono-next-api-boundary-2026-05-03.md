...
## Context
Harune now uses Hono for the app-owned API composition layer while keeping Next.js App Router as the public URL owner. Hono-backed API route handlers are mostly consolidated into one app-owned catch-all route handler:
...
|---|---|
| `/metadata` | `src/lib/api/routes/root.ts` |
