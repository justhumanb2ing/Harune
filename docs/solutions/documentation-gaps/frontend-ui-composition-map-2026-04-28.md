...
| `src/components/section/profile-page` | profile editor, preview, public renderer, route transition | 다른 도메인에서 재사용할 generic UI |
...
| authenticated user shell | `useUser()` / `meQueryOptions()` | sidebar/profile fallback |

## Why This Matters
Profile editor는 입력 즉시 DB에 쓰지 않는다. UI 컴포넌트가 React Query cache를 직접 draft처럼 바꾸거나, 개별 field mutation을 되살리면 현재 sync 모델과 충돌한다.

또한 공개 profile renderer와 editor preview renderer는 비슷하지만 목적이 다르다. Public renderer는 실제 링크 클릭/analytics/공개 동작을 포함하고, preview는 편집 중 시각화에 집중한다.

## When to Apply
- `src/components/section/profile-page/*`에서 draft 또는 preview 동작을 바꿀 때
...
3. 기존 *-section-editor.tsx
4. profile-page-preview.tsx
