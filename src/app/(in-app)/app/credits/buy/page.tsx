...
import { db } from "@/db";
import { type Quotas, plans } from "@/db/schema/plans";
import { users } from "@/db/schema/user";
import { env } from "@/env";
import { enableCredits } from "@/lib/credits/config";
import { type CreditType, creditBuyParams, getCreditsPrice } from "@/lib/credits/credits";
...
async function CreditsBuyPage({
  ...
  if (!enableCredits) {
    return redirect(
      `${env.NEXT_PUBLIC_APP_URL}/app/credits/buy/error?code=CREDITS_DISABLED&message=Credits are disabled`
    );
  }
  ...
  try {
    ...
    if (error instanceof z.ZodError) {
      return redirect(
        `${env.NEXT_PUBLIC_APP_URL}/app/credits/buy/error?code=INVALID_PARAMS&message=${error.message}`
      );
    }
    throw error;
  }
  ...
  let userPlan: { id: string; codename: string; quotas: Quotas } | undefined = undefined;
  if (user.planId) {
  ...
  try {
    ...
    return redirect(
      `${env.NEXT_PUBLIC_APP_URL}/app/credits/buy/error?code=PRICE_CALCULATION_ERROR&message=${encodeURIComponent(
  ...
  switch (provider) {
    ...
    case PlanProvider.STRIPE: {
      ...
      const stripeCheckoutSession = await stripe.checkout.sessions.create({
        ...
        customer_email: user.stripeCustomerId ? undefined : (session?.user?.email ?? undefined),
        ...
        success_url: `${env.NEXT_PUBLIC_APP_URL}/app/credits/buy/success?provider=${provider}&creditType=${creditType}&amount=${creditAmount}&sessionId={CHECKOUT_SESSION_ID}`,
        cancel_url: `${env.NEXT_PUBLIC_APP_URL}/app/credits/buy/cancel?provider=${provider}&creditType=${creditType}&amount=${creditAmount}&sessionId={CHECKOUT_SESSION_ID}`,
      });

      if (!stripeCheckoutSession.url) {
        console.error("Checkout session URL not created", stripeCheckoutSession);
    ...
    case PlanProvider.DODO: {
      ...
      const hasBillingInfo =
        billing_country && billing_state && billing_city && billing_street && billing_zipcode;

      // If not, redirect to the billing form
      if (!hasBillingInfo) {
        // Create the current URL as the callback URL
        const currentUrl = new URL("/app/credits/buy", env.NEXT_PUBLIC_APP_URL);
      ...
      const dodoProductId = env.DODO_CREDITS_PRODUCT_ID;
    ...
    case PlanProvider.PADDLE: {
      const paddleProductId = env.PADDLE_CREDITS_PRODUCT_ID;
      if (!paddleProductId) {
    ...
    default:
      return redirect(
        `${env.NEXT_PUBLIC_APP_URL}/app/credits/buy/error?code=UNSUPPORTED_PROVIDER&message=Payment provider not supported`
      );
  }
}
