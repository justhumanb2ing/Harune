...
import { getAllBlogs, getBlogBySlug, getRelatedBlogs } from "@/lib/mdx/blogs";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const ogImage = blog.frontmatter.featuredImage || seoConfig.defaultOgImagePath;
  ...
  return (
    <article className="py-16 md:py-32">
      ...
      <ArticleJsonLd
        ...
        title={blog.frontmatter.title}
        images={[absoluteUrl(blog.frontmatter.featuredImage || seoConfig.defaultOgImagePath)]}
        ...
        publisherLogo={absoluteUrl(seoConfig.defaultOgImagePath)}
      />
      <BreadcrumbJsonLd
        useAppDir
        itemListElements={[
        ...
        <div className="lg:grid lg:grid-cols-[1fr_280px] lg:gap-12">
          <main className="max-w-4xl">
            ...
            <header className="lg:hidden mb-6">
              <h1 className="text-3xl md:text-4xl font-semibold">{blog.frontmatter.title}</h1>
            </header>
            ...
            <div className="flex flex-wrap items-center gap-4 md:gap-6 text-sm text-muted-foreground mb-8 md:mb-10">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {blog.frontmatter.tags.map((tag) => (
                  <span key={tag} className="bg-muted px-3 py-1 rounded-full text-xs">
                    {tag}
                  </span>
                ))}
              </div>
            ...
            {relatedBlogs.length > 0 && (
              <section className="mt-16 md:mt-24" aria-labelledby="related-articles">
                <h2 id="related-articles" className="text-2xl font-semibold mb-8">
                  Related Articles
                </h2>
