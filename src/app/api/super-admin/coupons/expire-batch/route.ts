...
export const POST = withSuperAdminAuthRequired(async (req) => {
  try {
    ...
    await db.update(coupons).set({ expired: true }).where(inArray(coupons.code, codes));
    ...
    const affectedUserIds = [
      ...new Set(usedCoupons.filter((coupon) => coupon.userId).map((coupon) => coupon.userId)),
    ...
    for (const userId of affectedUserIds) {
      try {
        ...
        errors.push(`Failed to update plan for user ${userId}: ${errorMessage}`);
      }
    }
    ...
    if (notFoundCodes.length > 0) {
      errors.push(`Could not find the following coupon codes: ${notFoundCodes.join(", ")}`);
    }
