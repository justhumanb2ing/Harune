...
import { addCreditTransaction, getUserCredits } from "@/lib/credits/recalculate";
...
export const POST = withSuperAdminAuthRequired(async (req, context) => {
  ...
  try {
    ...
    const { action, creditType, amount, reason } = creditActionSchema.parse(body);
    ...
    return NextResponse.json({ error: "Failed to manage user credits" }, { status: 500 });
  }
});
