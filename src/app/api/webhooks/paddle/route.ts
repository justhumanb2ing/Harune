...
class PaddleWebhookHandler {
  ...
  async handleSubscriptionCreated() {
    ...
    if (!user) {
      console.log(`User not found for Paddle customer ID: ${subscription.customerId}`);
      return;
    }
  ...
  async handleSubscriptionUpdated() {
    ...
    if (subscription.status !== "active" && subscription.status !== "trialing") {
      console.log(`Subscription status is ${subscription.status}, skipping plan update/credits`);
  ...
  private async _handleCreditsPurchase(transaction: TransactionCompletedEvent["data"]) {
    ...
    const creditAmountRaw =
      typeof customData.creditAmount === "string" ? customData.creditAmount : null;
    ...
    const creditAmount = Number.parseInt(creditAmountRaw);
    ...
    try {
      await addCredits(userId, parsedCreditType.data as CreditType, creditAmount, transaction.id, {
        reason: "Purchase via Paddle",
        paddleTransactionId: transaction.id,
        amount: transaction.details?.totals?.total,
        currency: transaction.currencyCode,
      });
      console.log(`Successfully added ${creditAmount} ${creditTypeRaw} credits to user ${userId}`);
    } catch (error) {
      console.error("Error adding credits:", error);
      // If it's a duplicate payment error, that's okay - idempotency working
      if (error instanceof Error && error.message.includes("already exists")) {
        console.log(`Credits purchase already processed for transaction ${transaction.id}`);
      } else {
        throw error;
      }
    }
    ...
    if (!user && this.paddleClient) {
      try {
        ...
        if (customer?.email) {
          ...
          user = result.user;
          await db.update(users).set({ paddleCustomerId: customerId }).where(eq(users.id, user.id));
        }
      } catch (e) {
...
export async function POST(req: NextRequest) {
  ...
  if (!paddle) {
    return NextResponse.json({ error: "Paddle client not initialized" }, { status: 500 });
  }
