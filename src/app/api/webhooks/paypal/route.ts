...
class PayPalWebhookHandler {
  ...
  async handleOrderCompleted() {
    ...
    const [context] = await db
      .select()
      .from(paypalContext)
      .where(eq(paypalContext.paypalOrderId, orderId))
      .limit(1);
    if (!context) return;
    ...
    if (context.purchaseType === "credits" && context.creditType && context.creditAmount) {
      ...
      try {
        const creditAmount = Number.parseInt(context.creditAmount);
        ...
        await addCredits(userId, context.creditType as CreditType, creditAmount, orderId, {
        ...
        console.log(
          `Successfully added ${creditAmount} ${context.creditType} credits to user ${userId} via PayPal order ${orderId}`
        );
      } catch (error) {
    ...
    await db
      ...
      .set({ status: "success" })
      .where(eq(paypalContext.id, context.id));
  }
  ...
  async handleSubscriptionActivated() {
    ...
    const [context] = await db
      .select()
      .from(paypalContext)
      .where(eq(paypalContext.paypalSubscriptionId, subscriptionId))
      .limit(1);
    if (!context) return;
    const userId = context.userId;
  ...
  if (req.method === "POST") {
    try {
      ...
      console.log("[PayPal Webhook] Received event", {
        eventType,
        resourceType: event.resource_type || event.resource?.resource_type,
      ...
      try {
        ...
        switch (eventType) {
          ...
          case "PAYMENT.CAPTURE.COMPLETED":
            if (
              ...
              console.warn("[PayPal Webhook] Skipped: resource type mismatch for order", {
                resourceType,
              });
            }
            break;
          ...
          case "BILLING.SUBSCRIPTION.RE-ACTIVATED":
            if (resourceType === "subscription") {
              ...
              console.warn(
                "[PayPal Webhook] Skipped: resource type mismatch for subscription activated",
                { resourceType }
              );
            }
            break;
          ...
          case "BILLING.SUBSCRIPTION.EXPIRED":
            if (resourceType === "subscription") {
              ...
              console.warn(
                "[PayPal Webhook] Skipped: resource type mismatch for subscription cancelled",
                { resourceType }
              );
            }
            break;
        ...
        return NextResponse.json(
          { received: true, error: "Unexpected error processing webhook" },
          { status: 500 }
        );
      }
    } catch (error) {
