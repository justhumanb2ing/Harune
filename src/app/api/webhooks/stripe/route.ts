...
class StripeWebhookHandler {
  ...
  async handleCreditsPurchase(
    ...
    try {
      ...
      await addCredits(user.id, parsedCreditType.data as CreditType, creditAmount, paymentId, {
      ...
      console.log(
        `Successfully added ${creditAmount} ${creditType} credits to user ${user.id} via payment ${paymentId}`
      );
  ...
  async onSubscriptionCreated() {
    ...
    await db.update(users).set({ stripeSubscriptionId: object.id }).where(eq(users.id, user.id));
  ...
  async onCustomerCreated() {
    ...
    await db.update(users).set({ stripeCustomerId: object.id }).where(eq(users.id, user.id));
  }
