...
export function ExportCouponsModal({ currentFilter, searchQuery }: ExportCouponsModalProps) {
  ...
  const exportCoupons = async () => {
    ...
    try {
      ...
      const initialResponse = await apiFetch<CouponApiResponse>(
        `/api/super-admin/coupons?${params.toString()}`
      );
      ...
      for (let page = 1; page <= totalPages; page++) {
        ...
        const response = await apiFetch<CouponApiResponse>(
          `/api/super-admin/coupons?${params.toString()}`
        );
  ...
  const displayOptions =
    currentFilter === "all" && !searchQuery ? [exportOptions[0]] : exportOptions;
        ...
        {!isExporting ? (
          ...
          <div className="py-6 space-y-4">
            ...
            <p className="text-center text-sm text-muted-foreground">
              {progress}% Complete{" "}
              {exportedRecords > 0 ? `(${exportedRecords}/${totalRecords} records)` : ""}
            </p>
          </div>
        )}
      </DialogContent>
