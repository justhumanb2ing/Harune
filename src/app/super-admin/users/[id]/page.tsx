...
export default function UserDetailsPage() {
  ...
  const [creditType, setCreditType] = useState<"image_generation" | "video_generation">(
    "image_generation"
  );
  ...
  const {
    ...
    queryFn: ({ signal }) => apiFetch<UserDetails>(`/api/super-admin/users/${id}`, { signal }),
  });

  const { data: plansData } = useQuery({
    queryKey: queryKeys.superAdmin.plans.list({ limit: 100 }),
    queryFn: ({ signal }) =>
      apiFetch<{ plans: Plan[] }>("/api/super-admin/plans?limit=100", { signal }),
  });

  const { data: creditData } = useQuery({
    queryKey: queryKeys.superAdmin.users.credits({ id, limit: 10, page: creditPage }),
    queryFn: ({ signal }) =>
      apiFetch<CreditData>(`/api/super-admin/users/${id}/credits?page=${creditPage}&limit=10`, {
        signal,
      }),
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="flex flex-col gap-4">
            <div>
              ...
              <RadioGroup
                value={creditAction}
                onValueChange={(value: "add" | "deduct") => setCreditAction(value)}
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setCreditType(
                    (value ?? "image_generation") as "image_generation" | "video_generation"
                  )
                }
              >
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            <div className="space-y-4">
              <div>
                <Label htmlFor="current-plan" className="text-muted-foreground">
                  Current Plan
                </Label>
                <Select
              ...
              <dl className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                  <dt className="text-sm font-medium text-muted-foreground">Plan Name</dt>
                  <dd className="text-sm mt-1">{user?.currentPlan?.name || "No Plan"}</dd>
                </div>
                <div>
                  <dt className="text-sm font-medium text-muted-foreground">Plan Codename</dt>
                  <dd className="text-sm font-mono mt-1">{user?.currentPlan?.codename || "N/A"}</dd>
                </div>
                <div>
                  <dt className="text-sm font-medium text-muted-foreground">Stripe Subscription</dt>
                  <dd className="text-sm font-mono mt-1">{user?.stripeSubscriptionId || "N/A"}</dd>
                </div>
                <div>
        ...
        <div className="flex flex-row gap-6">
          <Card className="flex-1">
            <CardHeader>
              ...
              <CardDescription>User credit balances and transaction history.</CardDescription>
            </CardHeader>
            <CardContent>
                ...
                <TabsContent value="balance" className="space-y-4">
                  {creditData?.currentCredits ? (
                    <div className="flex flex-col gap-3">
                      {Object.entries(creditData.currentCredits).map(([type, amount]) => (
                        <div
                          key={type}
                        ...
                        >
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-medium">{formatCreditType(type)}</span>
                          </div>
                    ...
                    <>
                      ...
                      {creditData.pagination && (
                        <div className="flex items-center justify-between pt-4">
                          <p className="text-sm text-muted-foreground">
                            Page {creditData.pagination.page} of {creditData.pagination.totalPages}(
