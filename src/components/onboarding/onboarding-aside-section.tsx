...
const imageBoxesWithLayout: ImageBox[] = imageBoxes.map((box, index) => ({
  ...box,
  ...collageSlots[index],
}));

export default function OnboardingAsideSection() {
  return (
    <aside className="hidden h-full min-h-full xl:block xl:w-[clamp(18rem,22vw,26rem)] xl:shrink-0">
      <section className="relative h-full min-h-[860px]">
        <motion.div className="relative h-full w-full" viewport={{ once: true, margin: "-120px" }}>
          {imageBoxesWithLayout.map((image, index) => (
            <motion.div
              key={image.src}
              className={cn(
