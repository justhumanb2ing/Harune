...
import { GridResizeControls } from "@/components/profile/grid/grid-resize-controls";
import {
...
export function GridCard({
  ...
  const shellBackgroundClassName =
    item.itemType === "text" && textSurfaceClassNames
      ...
      : "bg-white";
  const wasLiftActiveRef = useRef(isLiftActive);

  useEffect(() => {
    if (!isSectionItem || readOnly) {
      return;
    }

    if (isLiftActive || wasLiftActiveRef.current) {
      setIsSectionPointerActive(false);
      setIsSectionFocusActive(false);
    }

    wasLiftActiveRef.current = isLiftActive;
  }, [isLiftActive, isSectionItem, readOnly]);
  ...
  return (
    <motion.div
      ...
      onPointerDownCapture={(event) => {
        ...
        if (!(target instanceof Element) || target.closest(GRID_CARD_INTERACTIVE_TARGET_SELECTOR)) {
          return;
        }
