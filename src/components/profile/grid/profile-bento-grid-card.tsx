...
import {
  createProfileBentoCardViewModel,
  getProfileBentoLinkSize,
  type ProfileBentoCardViewModel,
  type ProfileBentoClockCardViewModel,
  type ProfileBentoLinkCardViewModel,
...
import { formatClockTime, getClockTimeParts } from "@/lib/profile/clock";
...
export const ProfileBentoEditableContentCard = memo(function ProfileBentoEditableContentCard({
  ...
  return renderProfileBentoEditableCard(
    createProfileBentoCardViewModel({ activeBreakpoint, item, layoutSize }),
    {
      autoFocus,
...
function ClockBento({ viewModel }: { viewModel: ProfileBentoClockCardViewModel }) {
  ...
  return (
    <article
      ...
      style={{ backgroundColor: viewModel.backgroundColor }}
    >
      <div
        className={cn(
      ...
      >
        ...
        <span className="text-sm min-w-0 shrink truncate">{viewModel.timezoneLabel}</span>
      </div>
    </article>
  );
}
...
function LinkSupportingPanel({
  ...
  if (supportingPanel.kind === "github-contributions") {
    return <GithubContributionsPanel metadata={supportingPanel.metadata} className={className} />;
  }

  return <LinkThumbnail thumbnail={supportingPanel.thumbnail} className={className} />;
}
...
function ReadonlyLinkBento({
  ...
  viewModel: ProfileBentoLinkCardViewModel;
  preventNavigation: boolean;
}) {
  const { item, layoutSize, providerActionLabel, providerTheme, spotifyEmbedUri, supportingPanel } =
    viewModel;
    ...
    return (
      <article className="flex size-full min-h-0 gap-3 overflow-hidden rounded-lg p-2">
        ...
        <LinkSupportingPanel
          ...
          className="h-full w-[46%] min-h-0 min-w-0 shrink-0"
        />
      </article>
    );
  }
  ...
  if (layoutSize === "2x4") {
    return (
      <article className="flex size-full min-h-0 flex-col justify-between gap-3 overflow-hidden rounded-lg p-2">
        ...
        <LinkSupportingPanel
          supportingPanel={supportingPanel}
          className="h-[42%] w-full min-h-0 min-w-0 shrink-0"
        />
      </article>
    );
  }
  ...
  if (layoutSize === "2x4") {
    return (
      <article className="flex size-full min-h-0 flex-col justify-between gap-3 overflow-hidden rounded-lg p-2">
        ...
        <LinkSupportingPanel
          supportingPanel={supportingPanel}
          className="h-[42%] w-full shrink-0"
        />
      </article>
    );
  }
...
type ProfileBentoReadonlyCardRenderer<TType extends ProfileBentoType> = (props: {
  preventNavigation: boolean;
  viewModel: Extract<ProfileBentoCardViewModel, { type: TType }>;
}) => ReactNode;

type ProfileBentoEditableCardRenderer<TType extends ProfileBentoType> = (props: {
  autoFocus: boolean;
  mapInteractionEnabled: boolean;
  onChange: (item: ProfileBentoItem) => void;
  onFocusReady?: () => void;
...
const readonlyCardRenderers = {
  link: ({ preventNavigation, viewModel }) => (
    <ReadonlyLinkBento preventNavigation={preventNavigation} viewModel={viewModel} />
  ),
  text: ({ preventNavigation, viewModel }) => (
    <ReadonlyTextBento
      content={viewModel.item.content.content}
      preventNavigation={preventNavigation}
      url={viewModel.item.content.url}
    />
  ),
  section: ({ viewModel }) => <ReadonlySectionBento item={viewModel.item} />,
  media: ({ preventNavigation, viewModel }) => (
    <ReadonlyMediaBento item={viewModel.item} preventNavigation={preventNavigation} />
  ),
  map: ({ preventNavigation, viewModel }) => (
    <ReadonlyMapBento item={viewModel.item} preventNavigation={preventNavigation} />
  ),
  clock: ({ viewModel }) => <ClockBento viewModel={viewModel} />,
} satisfies {
  [TType in ProfileBentoType]: ProfileBentoReadonlyCardRenderer<TType>;
};

const editableCardRenderers = {
  link: ({ onChange, viewModel }) => (
    <EditableLinkBento onChange={onChange} viewModel={viewModel} />
  ),
  text: ({ autoFocus, onChange, onFocusReady, viewModel }) => (
    <EditableTextBento
      autoFocus={autoFocus}
      item={viewModel.item}
      onChange={onChange}
  ...
  section: ({ autoFocus, onChange, onFocusReady, viewModel }) => (
  ...
  media: ({ onChange, viewModel }) => (
    <EditableMediaBento item={viewModel.item} onChange={onChange} />
  ),
  map: ({ mapInteractionEnabled, onChange, viewModel }) => (
    <EditableMapBento
  ...
  [TType in ProfileBentoType]: ProfileBentoEditableCardRenderer<TType>;
};

function renderProfileBentoReadonlyCard(
  viewModel: ProfileBentoCardViewModel,
  options: { preventNavigation: boolean }
) {
  const renderer = readonlyCardRenderers[viewModel.type] as ProfileBentoReadonlyCardRenderer<
    typeof viewModel.type
  >;

  return renderer({ preventNavigation: options.preventNavigation, viewModel });
}

function renderProfileBentoEditableCard(
  viewModel: ProfileBentoCardViewModel,
  options: {
    autoFocus: boolean;
    mapInteractionEnabled: boolean;
  ...
  const renderer = editableCardRenderers[viewModel.type] as ProfileBentoEditableCardRenderer<
    typeof viewModel.type
  >;

  return renderer({ ...options, viewModel });
}
...
function ProfileBentoGridCardContent({
  ...
  return renderProfileBentoReadonlyCard(
    createProfileBentoCardViewModel({ activeBreakpoint, item, layoutSize }),
    { preventNavigation }
  );
}
