...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  useEffect(() => {
    ...
    const syncHandler = (handler: MapInteractionHandler | undefined, enabled: unknown) => {
      if (enabled === undefined || !handler) {
        return;
      }
    ...
    syncHandler(mapInstance.dragPan, dragPan);
    syncHandler(mapInstance.doubleClickZoom, doubleClickZoom);
    syncHandler(mapInstance.touchZoomRotate, touchZoomRotate);
  }, [mapInstance, dragPan, doubleClickZoom, touchZoomRotate]);
...
function ControlGroup({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex flex-col overflow-hidden rounded-md border border-black bg-black text-white shadow-sm [&>button:not(:last-child)]:border-b [&>button:not(:last-child)]:border-white/15">
      {children}
    </div>
  );
}
