      ...
      reorderPlaylistItems(activeId: string, overId: string) {
        setState((current) => {
          ...
          return recalculateDirtyState({
            ...
            draftData: {
              ...current.draftData,
              playlistItems: reorderItemsById(current.draftData.playlistItems, activeId, overId),
            },
            syncError: null,
          });
        });
          ...
          return recalculateDirtyState({
            ...
            draftData: {
              ...
              playlistItems: current.draftData.playlistItems.map((item) => ({
                ...item,
                blockPosition: blockPositions.get(playlistBlockId(item.id)) ?? item.blockPosition,
              })),
              textBoxItems: current.draftData.textBoxItems.map((item) => ({
                ...item,
