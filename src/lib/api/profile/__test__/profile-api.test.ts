...
type ProfileApiOptions = Parameters<typeof createProfileApi>[0];

const createTestProfileApi = (overrides: Partial<ProfileApiOptions> = {}) =>
  createProfileApi({
    ...
    deleteProfileMediaObject: async () => true,
    getProfileMediaPublicUrl: ({ contentHash, objectKey }) =>
      `https://media.example.com/${objectKey}?v=${contentHash}`,
    getProfileMediaObjectKeyFromUrl: (url) => new URL(url).pathname.replace(/^\/+/, ""),
    getTemporaryProfileBentoMediaObjectKey: ({ bentoId, userId }) =>
      `tmp/users/${userId}/profile/bento/${bentoId}/media-temp`,
    hashProfileMediaBuffer: (buffer) =>
      Array.from(new Uint8Array(buffer))
        .map((byte) => byte.toString(16).padStart(2, "0"))
        .join("")
        .padEnd(64, "0")
        .slice(0, 64),
    putProfileMediaObject: async () => {},
...
describe("profile page Hono API", () => {
  test("returns the existing unauthorized JSON contract when the session is missing", async () => {
    const app = createTestProfileApi({
      auth: async () => null,
    });
  ...
  test("maps ProfilePageError and unknown errors to the existing JSON response shapes", async () => {
    const profileErrorApp = createTestProfileApi({
      auth: async () => authenticatedSession,
      isHandleAvailableForUser: async () => {
    ...
    const unknownErrorApp = createTestProfileApi({
      auth: async () => authenticatedSession,
      isHandleAvailableForUser: async () => {
  ...
  test("uploads profile images to R2 with a stable user storage key", async () => {
    const uploadCalls: Array<{ body: Buffer; contentType: string; objectKey: string }> = [];
    const hash = "a".repeat(64);
    const app = createTestProfileApi({
      hashProfileMediaBuffer: () => hash,
      putProfileMediaObject: async (input) => {
    ...
    expect(response.status).toBe(200);
    expect(body).toEqual({
      imageUrl: `https://media.example.com/public/users/user-1/profile/profile?v=${hash}`,
    });
  ...
  test("validates profile image upload requests before writing to R2", async () => {
    ...
    const formData = new FormData();
    formData.append("file", new File(["avatar"], "avatar.gif", { type: "image/gif" }));
    formData.append("imageHash", "a".repeat(64));
    formData.append("imageKind", "profile");
  ...
  test("finalizes and deletes profile images for the authenticated user's storage prefix", async () => {
    const finalizedUrl = "https://cdn.example.com/public/users/user-1/profile/background?v=final";
    ...
    const app = createTestProfileApi({
      deleteProfileMediaObject: async (objectKey) => {
    ...
    expect(deletedObjectKeys).toEqual(["public/users/user-1/profile/background"]);
  });
  ...
  test("uploads profile bento media to a temporary object and returns the public preview URL", async () => {
    ...
    expect(body.tempObjectKey).toBe("tmp/users/user-1/profile/bento/bento-1/media-temp");
    expect(body.tempUrl).toBe(
      `https://media.example.com/tmp/users/user-1/profile/bento/bento-1/media-temp?v=${body.contentHash}`
    );
    expect(putCalls).toHaveLength(1);
    expect(putCalls[0]?.contentType).toBe("image/png");
    expect(putCalls[0]?.objectKey).toBe("tmp/users/user-1/profile/bento/bento-1/media-temp");
  });
