import { db } from "@/db";
import type { CreditTransaction } from "@/db/schema/credits";
 ...
 */
export async function allocatePlanCredits(params: AllocatePlanCreditsParams): Promise<void> {
  if (!enableCredits) {
    return;
  }
  ...
  try {
    ...
    if (!plan.length || !plan[0].codename) {
      console.log(`Plan ${planId} not found or has no codename, skipping credit allocation`);
      return;
    }
    ...
    if (!planCredits) {
      console.log(`No credit allocations configured for plan ${planCodename}, skipping`);
      return;
    }

    console.log(`Allocating credits for plan ${planCodename} (${planName}) to user ${userId}`);
    ...
    for (const [creditType, creditConfig] of creditAllocations) {
      try {
        ...
        console.error(`Error allocating ${creditType} credits for user ${userId}:`, error);
    ...
    console.log(`Completed credit allocation for plan ${planCodename} to user ${userId}`);
  } catch (error) {
