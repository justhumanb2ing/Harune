...
export async function recalculateUserCredits(userId: string): Promise<CreditRecord> {
  try {
    ...
    await db.update(users).set({ credits: creditBalances }).where(eq(users.id, userId));
...
export async function addCreditTransaction(
  ...
  try {
    ...
    await db.update(users).set({ credits: updatedCredits }).where(eq(users.id, userId));

    return updatedCredits;
  } catch (error) {
    console.error("Error adding credit transaction:", error);
    throw new Error(
      `Failed to add credit transaction for user ${userId}: ${error instanceof Error ? error.message : "Unknown error"}`
    );
  }
}
