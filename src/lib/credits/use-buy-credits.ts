...
import useCurrentPlan from "@/lib/users/use-current-plan";
import type { PlanProvider } from "../plans/get-subscribe-url";
import { creditsConfig } from "./config";
import { type CreditType, getCreditsBuyUrl, getCreditsPrice } from "./credits";
...
const useBuyCredits = (creditType: CreditType, amount: number): UseBuyCreditsResult => {
  ...
  const result = useMemo(() => {
    ...
    try {
      ...
      const validCurrentPlan =
        currentPlan?.codename && currentPlan.quotas
          ? {
              id: currentPlan.id,
