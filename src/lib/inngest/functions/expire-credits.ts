...
export const expireCredits = inngest.createFunction(
  ...
  async ({ step, logger }) => {
    ...
    const totalExpiringCount = await step.run("count-expiring-transactions", async () => {
    ...
    logger.info(
      `Processing ${totalExpiringCount} transactions in ${totalBatches} parallel batches of ${batchSize}`
    );
    ...
    const batchSteps = Array.from({ length: totalBatches }, (_, batchIndex) => {
      ...
      return step.run(
        ...
        async () => {
          ...
          const transactionPromises = expiringTransactions.map(async (transaction) => {
            try {
              ...
              logger.error(`Failed to expire credits for transaction ${transaction.id}:`, error);
