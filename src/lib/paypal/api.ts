...
export const PAYPAL_BASE_URL =
  env.NEXT_PUBLIC_PAYPAL_IS_SANDBOX === "true"
    ? "https://api-m.sandbox.paypal.com"
    : "https://api-m.paypal.com";

const base64EncodedCredentials = Buffer.from(
  `${env.NEXT_PUBLIC_PAYPAL_CLIENT_ID}:${env.PAYPAL_SECRET_KEY}`
).toString("base64");
...
export async function getPaypalAuthToken(forceRefresh = false) {
  ...
  if (!authToken || forceRefresh || new Date(authToken.expiresAt) < new Date()) {
    try {
      const newTokenData = await fetchNewToken();

      if (authToken) {
        await db.delete(paypalAccessTokens).where(eq(paypalAccessTokens.id, authToken.id));
      }
  ...
  const response = await fetch(`${PAYPAL_BASE_URL}/v2/checkout/orders`, {
    ...
    body: JSON.stringify({
      ...
      application_context: {
        ...
        user_action: "PAY_NOW",
        return_url: `${env.NEXT_PUBLIC_APP_URL}/app/subscribe/success?provider=paypal&paypalContextId=${contextId}`,
        cancel_url: `${env.NEXT_PUBLIC_APP_URL}/app/subscribe/error?provider=paypal&code=PAYPAL_CANCELLED&paypalContextId=${contextId}`,
      },
    }),
  });
  ...
  const response = await fetch(`${PAYPAL_BASE_URL}/v2/checkout/orders`, {
    ...
    body: JSON.stringify({
      ...
      application_context: {
        ...
        shipping_preference: "NO_SHIPPING",
        user_action: "PAY_NOW",
        return_url: `${env.NEXT_PUBLIC_APP_URL}/app/credits/buy/success?provider=paypal&creditType=${creditType}&amount=${creditAmount}&paypalContextId=${contextId}`,
        cancel_url: `${env.NEXT_PUBLIC_APP_URL}/app/credits/buy/cancel?provider=paypal&creditType=${creditType}&amount=${creditAmount}&paypalContextId=${contextId}`,
      },
    }),
  });
